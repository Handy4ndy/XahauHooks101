...
- Accepting or rejecting outgoing IOU (non-native token) payments based on amount, multiples, or thresholds
- Incoming payments and outgoing XAH payments are generally accepted unless explicitly rejected
- IOU amounts are handled using XFL (extended float) format conversions
- Hardcoded validation logic for outgoing IOU payments
...
|-----------------------------|---------------------|
| accept_outgoing_iou_exact.c  | Accepts outgoing IOU payments that exactly match 10 IOU, incoming payments, and outgoing XAH payments. |
| accept_outgoing_iou_max.c    | Accepts outgoing IOU payments below 100 IOU, incoming payments, and outgoing XAH payments. |
| accept_outgoing_iou_min.c    | Accepts outgoing IOU payments above 1 IOU, incoming payments, and outgoing XAH payments. |
| accept_outgoing_iou_multi.c  | Accepts outgoing IOU payments that are multiples of 10 IOU, incoming payments, and outgoing XAH payments. |
| accept_outgoing_iou_only.c   | Accepts only outgoing IOU payments, rejects incoming payments and outgoing XAH payments. |
| accept_outgoing_iou.c        | Accepts outgoing IOU payments and incoming payments, rejects outgoing XAH payments. |
