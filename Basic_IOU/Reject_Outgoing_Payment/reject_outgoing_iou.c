//**************************************************************
// Xahau Hook 101 Example ~ Reject Outgoing IOU
// Author: @Handy_4ndy
//
// Description:
...
//   - Outgoing XAH payments will be accepted; others rejected.
//
// Accepts:
...
int64_t hook(uint32_t reserved) {
    ...
    if (otxn_field(SBUF(amount), sfAmount) != 8)
        rollback(SBUF("ROI :: Rejected :: Incoming IOU payment."), __LINE__);

    // Accept incoming XAH payments
    accept(SBUF("ROI :: Accepted :: Incoming XAH payment."), __LINE__);
