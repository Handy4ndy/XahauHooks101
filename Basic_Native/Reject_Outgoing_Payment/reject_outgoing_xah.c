...
//   - All incoming payments and outgoing XAH payments will be rejected; outgoing IOU payments will be accepted.
//
// Accepts:
...
int64_t hook(uint32_t reserved) {
    ...
    if (otxn_field(SBUF(amount), sfAmount) == 8)
        rollback(SBUF("ROP :: Rejected :: Outgoing XAH payment."), __LINE__);

    // Accept outgoing IOU payments
    accept(SBUF("ROP :: Accepted :: Outgoing IOU payment."), __LINE__);
