//**************************************************************
// Xahau Hook 101 Example ~ Accept Outgoing Multi Hook
// Author: @handy_4ndy
//
// Description:
...
int64_t hook(uint32_t reserved) {
    ...
    if (otxn_drops == cost_drops)
        accept(SBUF("AOP :: Payment matches the required amount."), __LINE__);
    ...
    if (multi_count == 0)
        rollback(SBUF("AOP :: Payment is less than the required amount!"), __LINE__);

    TRACEVAR(multi_count);
    
    // If the payment is not an exact multiple, reject
    if (otxn_drops % cost_drops != 0)
        rollback(SBUF("AOP :: Payment amount must be a multiple of the required!"), __LINE__);

    // Accept the payment as it meets all criteria
    accept(SBUF("AOP :: Incoming XAH payment is a multiple of the required amount."), __LINE__);
