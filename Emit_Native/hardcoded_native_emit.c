//**************************************************************
// Xahau Hook 101 Example ~ Hardcoded Native Emit
// Author: @Handy_4ndy
//
// Description:
...
uint8_t ftxn_acc[20] = {0xE8U, 0xF1U, 0x0CU, 0xA2U, 0x7BU, 0xD8U, 0xECU, 0xDBU, 0x28U, 0x9BU, 0x94U, 0x7CU, 0xB1U, 0xA7U, 0x75U, 0x48U, 0xCAU, 0x0EU, 0x80U, 0xD9U};

int64_t hook(uint32_t reserved)
{

    TRACESTR("HNE :: Hardcoded Native Emit :: Called.");
    ...
    {
        accept(SBUF("HNE :: Accepted :  Outgoing payment transaction accepted"), __LINE__);
    }
    ...
    if (amount_len != 8)
    {
        rollback(SBUF("HNE :: Error: Non-XAH payment rejected."), __LINE__);
    }

    // Check if the payment is equal to the exact amount
    if (otxn_drops != exact_amount_drops)
    {
        rollback(SBUF("HNE :: Error: Payment amount doesn't match the exact_amount_value."), __LINE__);
    }
    ...
    {
        rollback(SBUF("HNE :: Error : Failed to emit transactions"), __LINE__);
    }

    accept(SBUF("HNE :: Accepted : Payment recieved and forwarded Successfully"), __LINE__);
