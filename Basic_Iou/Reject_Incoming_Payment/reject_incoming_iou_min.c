...
int64_t hook(uint32_t reserved) {
    ...
    if (amount_drops < min_drops)
        rollback(SBUF("RII :: Rejected :: Incoming IOU payment is below minimum amount."), __LINE__);

    // Accept the payment as it is above the minimum threshold
    accept(SBUF("RII :: Accepted :: Incoming IOU amount above the minimum amount threshold."), __LINE__);
