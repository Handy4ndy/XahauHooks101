...
int64_t hook(uint32_t reserved) {
    ...
    // Reject if not IOU
    if (amount_len != 48)
        rollback(SBUF("RIP :: Rejected :: Incoming XAH payment."), __LINE__);
    ...
    if (amount_drops == cost_drops)
        accept(SBUF("RII :: Accepted :: Payment matches the required amount."), __LINE__);
    ...
    if (multi_count == 0)
        rollback(SBUF("RII :: Rejected :: Payment is less than the required amount!"), __LINE__);

    // If the payment is not an exact multiple, reject
    if (amount_drops % cost_drops != 0)
        rollback(SBUF("RII :: Rejected :: Payment amount must be a multiple of the required!"), __LINE__);

    // Accept the payment as it meets all criteria
    accept(SBUF("RII :: Accepted :: Incoming IOU payment is a multiple of the required amount."), __LINE__);
