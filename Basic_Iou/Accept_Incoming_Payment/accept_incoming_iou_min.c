//**************************************************************
// Xahau Hook 101 Example ~ Accept Incoming IOU Min Hook
// Author: @handy_4ndy
//
// Description:
//   This hook accepts incoming IOU payments when the amount is above the minimum threshold.
//   The minimum amount is set to 1 IOU.
//
// Accepts:-
//   - Incoming IOU payments when the amount is above the minimum threshold.
//   - Outgoing payments.
...
int64_t hook(uint32_t reserved) {
    ...
    if (amount_drops < min_drops)
        rollback(SBUF("AII :: Incoming payment is below minimum amount."), __LINE__);

    // Accept the payment as it is above the minimum threshold
    accept(SBUF("AII :: Incoming IOU amount above the minimum amount threshold."), __LINE__);
