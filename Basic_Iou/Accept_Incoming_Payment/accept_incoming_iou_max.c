//**************************************************************
// Xahau Hook 101 Example ~ Accept Incoming IOU Max Hook
// Author: @handy_4ndy
//
// Description:
//   This hook accepts incoming IOU payments when the amount is below the maximum threshold.
//   The maximum amount is set to 100 IOU.
//
// Accepts:-
//   - Incoming IOU payments when the amount is below the maximum threshold.
//   - Outgoing payments.
...
int64_t hook(uint32_t reserved) {
    ...
    if (amount_drops > max_drops)
        rollback(SBUF("AII :: Incoming payment above maximum amount."), __LINE__);

    // Accept the payment as it is below the maximum threshold
    accept(SBUF("AII :: Incoming IOU amount below the maximum amount threshold."), __LINE__);
