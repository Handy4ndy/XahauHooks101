//**************************************************************
// Xahau Hook 101 Example ~ HMIE :: Hardcoded Multi-Invoke Emit
// Author: @Handy_4ndy
//
// Description:
//   Emits three Invoke transactions, each to a different hardcoded destination,
//   whenever an incoming Payment transaction is detected. The transaction buffer
//   is reused, with the destination field redefined for each emit. This demonstrates
//   the minimal, canonical pattern for emitting to multiple hardcoded accounts.
//
// Reference: HMIE (Hardcoded Multi-Invoke Emit)
//
// Triggers:
//   ttPAYMENT (incoming payments only)
//
// Parameters:
//   None (all values, including destinations, are hardcoded for demonstration)
//
// Usage:
//   - Deploy the hook to your account.
//   - Any incoming XAH payment will trigger three emitted Invoke transactions.
//
// Accepts:
//   - All incoming XAH payments
//
// Emits:
//   - Three hardcoded Invoke transactions, one to each of the three hardcoded destinations
//   - The destination is set by redefining the destination field in the transaction buffer before each emit
//**************************************************************
...
int64_t hook(uint32_t reserved)
{
    ...
    TRACESTR("HMIE :: Hardcoded Multi-Invoke Emit :: Called");

    // Only trigger on Payment transactions
    int64_t tt = otxn_type();
    if (tt != 0) // 0 = ttPAYMENT
        accept(SBUF("HMIE :: Success :: Not a payment, skipping."), __LINE__);
    ...
    // Define three hardcoded destination accounts (identical for demonstration)
    uint8_t dest1[20] = {0xF7U, 0x34U, 0xDAU, 0xE9U, 0xFBU, 0x86U, 0xA7U, 0xEAU, 0x54U, 0x3BU, 0xBFU, 0xFEU, 0xCBU, 0xF4U, 0x32U, 0xF5U, 0x0DU, 0x2BU, 0x64U, 0x23U};
    uint8_t dest2[20] = {0xF7U, 0x34U, 0xDAU, 0xE9U, 0xFBU, 0x86U, 0xA7U, 0xEAU, 0x54U, 0x3BU, 0xBFU, 0xFEU, 0xCBU, 0xF4U, 0x32U, 0xF5U, 0x0DU, 0x2BU, 0x64U, 0x23U};
    uint8_t dest3[20] = {0xF7U, 0x34U, 0xDAU, 0xE9U, 0xFBU, 0x86U, 0xA7U, 0xEAU, 0x54U, 0x3BU, 0xBFU, 0xFEU, 0xCBU, 0xF4U, 0x32U, 0xF5U, 0x0DU, 0x2BU, 0x64U, 0x23U};
    ...
    // Emit to dest1 (redefine destination in txn buffer)
    SET_ACCOUNT(DEST_OUT, dest1);
    PREPARE_TXN();
    uint8_t emithash1[32];
    int64_t emit_result1 = emit(SBUF(emithash1), SBUF(txn));
    if (emit_result1 != 32)
        rollback(SBUF("HMIE :: Error :: Emit 1 failed."), __LINE__);
    TRACEHEX(emithash1);

    // Emit to dest2 (redefine destination in txn buffer)
    SET_ACCOUNT(DEST_OUT, dest2);
    PREPARE_TXN();
    uint8_t emithash2[32];
    int64_t emit_result2 = emit(SBUF(emithash2), SBUF(txn));
    if (emit_result2 != 32)
        rollback(SBUF("HMIE :: Error :: Emit 2 failed."), __LINE__);
    TRACEHEX(emithash2);

    // Emit to dest3 (redefine destination in txn buffer)
    SET_ACCOUNT(DEST_OUT, dest3);
    PREPARE_TXN();
    uint8_t emithash3[32];
    int64_t emit_result3 = emit(SBUF(emithash3), SBUF(txn));
    if (emit_result3 != 32)
        rollback(SBUF("HMIE :: Error :: Emit 3 failed."), __LINE__);
    TRACEHEX(emithash3);

    accept(SBUF("HMIE :: Success :: Payment received, 3 invokes emitted."), __LINE__);
