//**************************************************************
// Xahau Hook 101 Example ~ IIE :: Install-time Invoke Emit
// Author: @Handy_4ndy
//
// Description:
//   Emits a single Invoke transaction to a destination specified by an install-time
//   hook parameter (DEST) whenever an incoming Payment transaction is detected.
//   This demonstrates canonical install-time parameterization for Hooks 101.
//
// Triggers:
//   ttPAYMENT (incoming payments only)
//
// Parameters:
//   DEST (20 bytes, required): Destination account to receive the emitted Invoke transaction.
//     - Set at install time using the Hook Parameter interface.
//
// Usage:
//   - Deploy the hook to your account, specifying the DEST parameter.
//   - Any incoming XAH payment will trigger an emitted Invoke transaction to DEST.
//
// Accepts:
//   - All incoming XAH payments
//
// Emits:
//   - A single Invoke transaction to the install-time DEST parameter
//   - Fails if DEST is not set or invalid
//
//**************************************************************
...
int64_t hook(uint32_t reserved)
{
    ...
    TRACESTR("IIE :: Install-time Invoke Emit :: Called");
    ...
    // Fetch DEST parameter from install-time hook parameters
    uint8_t dest_buf[20];
    if (hook_param(SBUF(dest_buf), "DEST", 4) != 20)
        rollback(SBUF("IIE :: Error :: DEST parameter not set or invalid."), __LINE__);
    // Set destination to the install-time parameter
