//**************************************************************
// Xahau Hook 101 Example ~ Hardcoded Multi URI Remit
// Author: @Handy_4ndy
//
// Description:
...
int64_t hook(uint32_t reserved) {
    TRACESTR("HMUR :: Hardcoded Multi URI Remit :: Called.");
    ...
    if (tt != 0) {
        accept(SBUF("HMUR :: Not a Payment. Accepting."), __LINE__);
    }

    // Reserve space for 2 emissions upfront
    etxn_reserve(2);

    // Fixed URI examples
    uint8_t uri[] = "ipfs://bafybeifhrrj3pvotihooenwljj4rreskvdb6dahxwq7cskpao73ishk2p4/000002.json";
    uint64_t uri_len = sizeof(uri) - 1; 

    uint8_t uri2[] = "ipfs://bafybeifhrrj3pvotihooenwljj4rreskvdb6dahxwq7cskpao73ishk2p4/000003.json";
    uint64_t uri_len2 = sizeof(uri2) - 1; 
    ...
    int64_t emit_result1 = emit(SBUF(emithash), txn, BYTES_LEN + uri_len + 1);
    if (emit_result1 > 0) {
        successful_emissions++;
    }

    // Emit second URI
    PREPARE_REMIT_TXN(hook_acc, otx_acc, uri2, uri_len2);
    int64_t emit_result2 = emit(SBUF(emithash), txn, BYTES_LEN + uri_len2 + 1);
    if (emit_result2 > 0) {
        successful_emissions++;
    }

    // Check results
    if (successful_emissions == 2) {
        accept(SBUF("HMUR :: Success :: Both URITokens minted!"), __LINE__);
    } else if (successful_emissions == 1) {
        accept(SBUF("HMUR :: Partial Success :: 1 of 2 URITokens minted!"), __LINE__);
    }

    rollback(SBUF("HMUR :: Error :: No URITokens could be minted."), __LINE__);
