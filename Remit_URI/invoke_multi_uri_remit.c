//**************************************************************
// Xahau Hook 101 Example ~ Invoke Multi URI Remit
// Author: @Handy_4ndy
//
// Description:
...
//   - Incoming payments mint MINT sequential URITokens until COUNT reaches zero.
...
int64_t hook(uint32_t reserved) {
    TRACESTR("IMUR :: Invoke Multi URI Remit :: Called.");
    ...
    if (tt == 99) {
        if (!equal) {
            rollback(SBUF("IMUR :: Error :: Only hook owner can configure!"), __LINE__);
        }
        ...
        if (prefix_len > 0) {
            if (state_set(prefix_buffer, prefix_len, SBUF(prefix_key)) < 0) {
                rollback(SBUF("IMUR :: Error :: Could not set PREFIX state!"), __LINE__);
            }
        }
        ...
        if (count_param_len == 8) {
            ...
            if (state_set(SBUF(count_param_buf), SBUF(count_key)) < 0) {
                rollback(SBUF("IMUR :: Error :: Could not set COUNT state!"), __LINE__);
            }

            // Also set TOTAL state (for numbering sequence)
            if (state_set(SBUF(count_param_buf), SBUF(total_key)) < 0) {
                rollback(SBUF("IMUR :: Error :: Could not set TOTAL state!"), __LINE__);
            }
        ...
        uint8_t mint_param_buf[8];
        int64_t mint_param_len = otxn_param(SBUF(mint_param_buf), SBUF(mint_key));

        if (mint_param_len == 8) {
            uint64_t mint_value = UINT64_FROM_BUF(mint_param_buf);

            // Cap at 5 maximum
            if (mint_value > 5) {
                mint_value = 5;
                UINT64_TO_BUF(mint_param_buf, mint_value);
            }
            ...
            // Set MINT state
            if (state_set(SBUF(mint_param_buf), SBUF(mint_key)) < 0) {
                rollback(SBUF("IMUR :: Error :: Could not set MINT state!"), __LINE__);
            }
        }

        accept(SBUF("IMUR :: Configuration updated successfully!"), __LINE__);
    }

    // PAYMENT TRANSACTION (tt=0) - Mint sequential URIToken
    if (tt == 0) {
        ...
        if (prefix_len <= 0) {
            rollback(SBUF("IMUR :: Error :: No URI prefix configured! Use invoke to set PREFIX."), __LINE__);
        }

        uint8_t count_buf[8];
        if (state(SBUF(count_buf), SBUF(count_key)) < 0) {
            rollback(SBUF("IMUR :: Error :: No count configured! Use invoke to set COUNT."), __LINE__);
        }
        ...
        if (remaining_count <= 0) {
            rollback(SBUF("IMUR :: Error :: No NFTs remaining to mint!"), __LINE__);
        }

        // Get total count for sequence numbering
        uint8_t total_buf[8];
        if (state(SBUF(total_buf), SBUF(total_key)) < 0) {
            rollback(SBUF("IMUR :: Error :: Could not get total count!"), __LINE__);
        }

        uint64_t total_count = UINT64_FROM_BUF(total_buf);

        // Get MINT count (default to 1 if not set)
        uint8_t mint_buf[8];
        uint64_t mint_count = 1; // default
        if (state(SBUF(mint_buf), SBUF(mint_key)) >= 0) {
            mint_count = UINT64_FROM_BUF(mint_buf);
        }

        // Ensure we don't mint more than available
        if (mint_count > remaining_count) {
            mint_count = remaining_count;
        }
        ...
        TRACESTR("Minting multiple sequential URITokens...");

        // Reserve space for all emissions
        etxn_reserve(mint_count);

        uint8_t emithash[32];
        uint64_t successful_emissions = 0;

        // Mint the specified number of sequential URITokens
        for (uint64_t mint_i = 0; GUARD(25), mint_i < mint_count; ++mint_i) {
            uint64_t current_number = total_count - remaining_count + 1 + mint_i;
            ...
            if (emit_result > 0) {
                ...
                TRACESTR("Failed to mint URIToken!");
            }
        }

        // Update remaining count by successful emissions
        if (successful_emissions > 0) {
            remaining_count -= successful_emissions;
            UINT64_TO_BUF(count_buf, remaining_count);

            if (state_set(SBUF(count_buf), SBUF(count_key)) < 0) {
                rollback(SBUF("IMUR :: Error :: Could not update count state!"), __LINE__);
            }

            TRACEVAR(successful_emissions);
            TRACEVAR(remaining_count);

            if (successful_emissions == mint_count) {
                accept(SBUF("IMUR :: Success :: All sequential URITokens minted!"), __LINE__);
            } else {
    ...
    accept(SBUF("IMUR :: Transaction accepted."), __LINE__);
